const createRides = (array) => {
  ...
  array.forEach((item) => {
    ...
    <div id="card-body--${item.firebaseKey}" class="card-body" style="height: 400px; width: 350px; background-image: url('${item.rideImageURL}'); border-radius: 18px; background-size: Cover; background-repeat: no-repeat;">
      ...
      <button class="btn btn-outline-danger" id="delete-ride--${item.firebaseKey}" style="position: absolute; right:0; bottom: 0; color: #b1bac4; border: none;">🗑️</button>
      <button class="btn" id="edit-ride--${item.firebaseKey}" style="position: absolute; left:0; bottom: 0; color: #b1bac4; border: none;" data-toggle="modal" data-target="#formModal">Edit Ride</button>
  </div>`;
  });
};
