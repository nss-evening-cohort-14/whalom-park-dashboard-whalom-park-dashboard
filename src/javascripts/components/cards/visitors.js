const createVisitors = (visitorArray) => {
  ...
  visitorArray.forEach((card) => {
    ...
    <div id="card-body--${card.firebaseKey}" class="card-body" style="height: 400px; width: 350px; background-image: url('${card.visitorImageURL}'); border-radius: 18px; background-size: Cover; background-repeat: no-repeat;">
      ...
      <button class="btn btn-outline-danger" id="delete-visitor--${card.firebaseKey}" style="position: absolute; right:0; bottom: 0; color: #b1bac4; border: none;">🗑️</button>
      <button class="btn" id="active-staff--${card.visitorID_firebaseKey}" style="position: absolute; left:0; bottom: 0; color: #b1bac4; border: none;"><img src=""</button>
  </div>`;
  });
};
